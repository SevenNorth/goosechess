...
Pixi Scene 不得自行修改玩家位置。它只根据状态机给出的路径播放动画。
...
1. 状态机先计算骰子、目标位置和包含折返的完整移动路径。
2. PixiJS 沿路径从当前格向目标格绘制预览线，完成后发出 `ROUTE_PREVIEW_DONE`。
3. PixiJS 在目标格绘制强调圆圈，随后隐藏路线并发出 `TARGET_EMPHASIS_DONE`。
4. PixiJS 按路径播放逐格跳跃动画。
5. PixiJS 发出 `MOVE_ANIMATION_DONE`。
6. 状态机首先检查最终落点是否获胜；未获胜时才进入碰撞和事件处理。

事件效果产生位置变化后也必须进入 `landing.winCheck`，不能等到回合结束才判断胜利。

任何动画失败或页面失焦都必须能通过超时回退继续游戏，不能永久阻塞状态机。

## 5. 随机数与回放

- 每局游戏创建一个整数种子。
- 所有骰子、事件抽取和电脑随机偏差使用同一随机数服务。
- 日志记录玩家命令和随机结果。
- 开发模式允许输入种子重新开始同一局。
...
- 场景测试：检查棋盘成功渲染、棋子坐标有效、动画完成事件会返回状态机。
