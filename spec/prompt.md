...
- `main` 在编写本交接前最新提交为 `3546e8c chore: 忽略内容服务本地环境配置`；开始工作时必须重新核对实际 HEAD、工作区和远端同步状态。
...
- 管理员内容平台已经完成认证/RBAC 与内容生命周期基础：`apps/content-server` 具备 SQLite 草稿、不可变修订、自动校验、审核、发布、回滚和审计 API，`packages/content-tools` 负责事件、地图和皮肤的结构校验、规范化 JSON 与哈希；`apps/admin` 尚未建立。
...
- 下一实施优先级：独立 `apps/admin` 登录外壳与事件编辑器 → 地图编辑器 → 皮肤制作页面 → 默认内容导入和游戏服务发布版本接入。
- 管理端 Web 必须调用已有受保护 API；先用事件编辑器验证完整工作流，再扩展地图和皮肤页面。
