...
- `main` 在编写本交接前最新提交为 `83638aa feat: 建立管理员内容生命周期基础`，当时比 `origin/main` 领先 1 个提交；开始工作时必须重新核对实际 HEAD、工作区和远端同步状态。
...
- 本次建议直接建立 `apps/admin`：先完成登录、会话恢复、退出、角色路由守卫和安静紧凑的管理布局，再实现事件草稿列表、结构化编辑、自动校验结果、提交审核、管理员审核/发布以及版本和审计查看。`content-editor` 与 `admin` 的可见操作必须匹配服务端权限，前端隐藏按钮不能代替服务端校验。
