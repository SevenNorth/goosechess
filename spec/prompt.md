...
- `main` 在编写本交接前与 `origin/main` 同步，最近的业务提交为 `69d9c14 docs: 规划管理员内容平台`；开始工作时必须重新核对实际 HEAD。
...
- 在线阶段尚未完成多实例共享租约与房间分片，以及监控、限流、协议指标、异常对局诊断和跨浏览器/高延迟发布验收。
- 管理员内容平台目前只有规格规划，尚未开始实现。规划位置是独立的 `apps/admin`、`apps/content-server` 和 `packages/content-tools`，复用 `packages/game-content`；实施顺序为最小账号权限、内容生命周期基础、事件编辑器、地图编辑器、皮肤制作页面。
...
- 如果我没有给出新的功能方向，先完成在线对局阶段剩余的生产化工作，再进入账号与管理员内容平台。
- 在线生产化优先顺序：可观测性与限流基础 → 多实例房间所有权/租约 → 跨浏览器和高延迟 E2E → 发布验收。
- 管理员平台开始实施时，先做最小身份认证和服务端 RBAC，再建立草稿、校验、审核、发布、回滚和审计模型；不要直接从三个编辑页面之一开始堆界面。
