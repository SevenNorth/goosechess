...
import { createDefaultMap, csvValues, integerCsvValues, localMapIssues, mapFromUnknown, simulateMapPath } from './map-model'
...
type Landmark = MapDefinition['landmarks'][number]
...
function MapForm({ map, selectedSpaceId, disabled, onSelectSpace, onChange }: {
  ...
  const patchLandmark = (id: string, values: Partial<Landmark>) => patch({ landmarks: map.landmarks.map((landmark) => landmark.id === id ? { ...landmark, ...values } : landmark) })
  const landmarkPools = map.landmarkEventPoolIds ?? {}
  ...
  function removeLastSpace() {
    ...
    patch({
      ...
      landmarks: map.landmarks.map((landmark) => ({ ...landmark, spaceIds: landmark.spaceIds.filter((id) => id !== removed) })),
    })
    ...
    <div className="map-form">
      ...
      <section className="form-section">
        ...
        <div className="form-grid space-property-grid">
          ...
          <label><span>地标</span><select value={selected.landmarkId ?? ''} disabled={disabled} onChange={(event) => patchSpace({ landmarkId: event.target.value || undefined })}><option value="">无</option>{map.landmarks.map((landmark) => <option key={landmark.id} value={landmark.id}>{landmark.name}</option>)}</select></label>
        </div>
      </section>

      <section className="form-section">
        <div className="section-title"><span>03</span><div><strong>地标配置</strong><small>地标与关联格必须保持一致</small></div></div>
        <div className="landmark-editor-list">
          {map.landmarks.map((landmark) => <div className="landmark-editor-row" key={landmark.id}><input aria-label={`${landmark.name} ID`} value={landmark.id} disabled /><input aria-label={`${landmark.id} 名称`} value={landmark.name} disabled={disabled} onChange={(event) => patchLandmark(landmark.id, { name: event.target.value })} /><input aria-label={`${landmark.id} 格子`} value={landmark.spaceIds.join(', ')} disabled={disabled} onChange={(event) => patchLandmark(landmark.id, { spaceIds: integerCsvValues(event.target.value) })} /><input aria-label={`${landmark.id} X`} type="number" value={landmark.x ?? ''} placeholder="X" disabled={disabled} onChange={(event) => patchLandmark(landmark.id, { x: event.target.value ? Number(event.target.value) : undefined })} /><input aria-label={`${landmark.id} Y`} type="number" value={landmark.y ?? ''} placeholder="Y" disabled={disabled} onChange={(event) => patchLandmark(landmark.id, { y: event.target.value ? Number(event.target.value) : undefined })} /><input aria-label={`${landmark.id} 尺寸`} type="number" value={landmark.size ?? ''} placeholder="尺寸" disabled={disabled} onChange={(event) => patchLandmark(landmark.id, { size: event.target.value ? Number(event.target.value) : undefined })} /></div>)}
        </div>
      </section>

      <section className="form-section">
        <div className="section-title"><span>04</span><div><strong>事件池与资源</strong><small>限定地图可抽取内容与预览素材</small></div></div>
        ...
        <label><span>通用事件池</span><textarea rows={2} value={(map.genericEventPoolIds ?? []).join(', ')} disabled={disabled} onChange={(event) => patch({ genericEventPoolIds: csvValues(event.target.value) })} /></label>
        <div className="landmark-pool-list">{map.landmarks.map((landmark) => <label key={landmark.id}><span>{landmark.name}</span><input value={(landmarkPools[landmark.id] ?? []).join(', ')} disabled={disabled} onChange={(event) => patch({ landmarkEventPoolIds: { ...landmarkPools, [landmark.id]: csvValues(event.target.value) } })} /></label>)}</div>
        <div className="form-grid two-columns"><label><span>棋盘背景</span><input value={map.assets.background} disabled={disabled} onChange={(event) => patch({ assets: { ...map.assets, background: event.target.value } })} /></label><label><span>地标图集</span><input value={map.assets.landmarkAtlas} disabled={disabled} onChange={(event) => patch({ assets: { ...map.assets, landmarkAtlas: event.target.value } })} /></label></div>
      </section>
    </div>
  )
}

function MapInspector({ map, selectedSpaceId, onSelectSpace, validation }: {
  readonly map: MapDefinition
  readonly selectedSpaceId: number
  readonly onSelectSpace: (id: number) => void
  ...
  return (
    <aside className="map-inspector">
      <div className="panel-heading"><div><small>PIXΙ PREVIEW</small><strong>地图预览</strong></div><MapPinned /></div>
      <MapPreview map={map} selectedSpaceId={selectedSpaceId} path={simulation?.path ?? []} onSelectSpace={select} />
...
export function MapWorkspace() {
  ...
  useEffect(() => {
    let active = true
    setError(null); setNotice(null); setSelectedSpaceId(0)
  ...
  function updateDraft(next: ContentDraft) {
    setDraft(next); setMap(mapFromUnknown(next.content)); setDirty(false)
  ...
  const changeMap = (next: MapDefinition) => { setMap(next); setDirty(true); setNotice(null) }
  return (
    <div className="map-workspace">
      ...
      {draftId ? <MapInspector map={map} selectedSpaceId={selectedSpaceId} onSelectSpace={setSelectedSpaceId} validation={draft?.validation} /> : null}
    </div>
  )
}
